...
function suggestSessionsFromInput(sessions: SupportSession[], input: string): SupportSession[] {
  const terms = input
    .trim()
    .toLowerCase()
    .split(/\s+/)
  ...
  return sessions.filter((s) => terms.some((term) => searchable(s).includes(term)));
}
...
function EnrollmentCount({
  ...
  useEffect(() => {
    if (pulse) {
      scale.value = withSequence(withTiming(1.2, { duration: 150 }), withTiming(1, { duration: 200 }));
    }
  }, [pulse, count]);
  ...
  return (
    <Animated.View style={[styles.enrollmentRow, animatedStyle]}>
      <MaterialIcons name="groups" size={18} color={accentColor} style={styles.enrollmentIcon} />
  ...
  return (
    <Pressable
      ...
      <View style={styles.cardHeader}>
        <ThemedText type="defaultSemiBold" style={[styles.cardTitle, { color: colors.text }]} numberOfLines={2}>
          {session.name}
        </ThemedText>
        <View style={[styles.formatBadge, { backgroundColor: colors.mint }]}>
          ...
          />
          <ThemedText style={[styles.formatText, { color: accent }]}>{session.format}</ThemedText>
        </View>
      </View>
      <ThemedText style={[styles.cardProvider, { color: muted }]}>{session.providerName}</ThemedText>
      ...
      <View style={styles.metaRow}>
        ...
        <ThemedText style={[styles.cardGroupType, { color: muted }]}>{session.availability}</ThemedText>
      </View>
      <View style={styles.metaRow}>
        <MaterialIcons name="language" size={16} color={muted} />
        <ThemedText style={[styles.cardGroupType, { color: muted }]}>{languageLabel}</ThemedText>
      </View>
      <EnrollmentCount
...
export default function HomeSessionsScreen() {
  ...
  const [therabotSuggestions, setTherabotSuggestions] = useState<SupportSession[]>([]);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <ScrollView
        ...
        <Animated.View entering={FadeInDown.duration(400).delay(120)} style={styles.list}>
          {filtered.length === 0 ? (
            ...
            filtered.map((session) => (
              <SessionCard
                ...
                enrolledLabel={session.enrolledCount === 1 ? t('home.enrolledOne') : t('home.enrolled')}
                languageLabel={t(`session.language.${session.language.toLowerCase()}`)}
              />
            ))
          )}
        </Animated.View>
      ...
      <Pressable
        style={[styles.aiFab, { backgroundColor: colors.primary }, shadow.cardWarm]}
        onPress={() => {
          Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
      ...
      <Modal visible={therabotModalOpen} transparent animationType="fade">
        <Pressable style={styles.aiOverlay} onPress={() => setTherabotModalOpen(false)}>
          <Pressable
            style={[styles.aiCard, { backgroundColor: colors.surface }, shadow.card]}
            onPress={(e) => e.stopPropagation()}>
            <ThemedText type="defaultSemiBold" style={[styles.aiTitle, { color: colors.text }]}>
              {t('therabot.title')}
            </ThemedText>
            ...
            />
            <Pressable
              style={[styles.therabotSuggestBtn, { backgroundColor: colors.primary }]}
              onPress={() => {
                Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
              ...
              <ThemedText style={styles.therabotSuggestBtnLabel}>{t('therabot.suggest')}</ThemedText>
            </Pressable>
            {therabotSuggestions.length > 0 ? (
              <>
                <ThemedText style={[styles.therabotSuggestionsLabel, { color: colors.textMuted }]}>
                ...
                <View style={styles.aiSuggestions}>
                  {therabotSuggestions.map((s) => (
                    <Pressable
                      key={s.id}
                      style={[styles.aiSuggestionChip, { borderColor: colors.primary }]}
                      onPress={() => {
                        setTherabotModalOpen(false);
                        openSession(s.id);
                      }}>
                      <ThemedText style={[styles.aiSuggestionText, { color: colors.primary }]} numberOfLines={1}>
                        {s.name}
                      </ThemedText>
                    </Pressable>
                  ))}
              ...
              <ThemedText style={[styles.therabotNoResults, { color: colors.textMuted }]}>
            ...
            <Pressable
              ...
              <ThemedText style={[styles.aiCloseLabel, { color: colors.textMuted }]}>{t('therabot.close')}</ThemedText>
            </Pressable>
          </Pressable>
        </Pressable>
      </Modal>
...
const styles = StyleSheet.create({
  ...
  hero: { marginBottom: spacing.lg, paddingVertical: spacing.md, paddingHorizontal: spacing.lg, borderRadius: radius.card },
  ...
  enrollmentRow: { flexDirection: 'row', alignItems: 'center', marginTop: spacing.sm },
  ...
  aiOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'center', padding: spacing.lg },
  ...
  therabotSuggestBtnLabel: { fontSize: 16, fontWeight: '600', color: '#0F172A' },
  ...
  aiSuggestionChip: { paddingVertical: 12, paddingHorizontal: 14, borderRadius: 12, borderWidth: 1 },
  aiSuggestionText: { fontSize: 14 },
  aiClose: { alignSelf: 'center', paddingVertical: 10, paddingHorizontal: 20, borderRadius: 999, borderWidth: 1 },
