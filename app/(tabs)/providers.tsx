...
function ProviderCard({
  ...
  const degree =
    locale === "ar" && provider.degreeAr ? provider.degreeAr : provider.degree;
  const specialization =
    locale === "ar" && provider.specializationAr
      ? provider.specializationAr
    ...
    >
      <View style={styles.cardInner}>
        ...
        <View style={styles.cardBody}>
          ...
          <ThemedText
            style={[styles.credentials, { color: colors.textMuted }]}
            numberOfLines={2}
          >
            {degree}
          </ThemedText>
          <View style={styles.chipRow}>
            {specialties.map((spec) => (
              <View
                key={spec}
                style={[styles.chip, { backgroundColor: colors.mint }]}
              >
                <ThemedText
          ...
          {provider.volunteerCoHost && (
            ...
            >
              <ThemedText
                style={[styles.volunteerText, { color: colors.textMuted }]}
              >
                {volunteerLabel}
              </ThemedText>
            </View>
  ...
  return (
    ...
    >
      <Animated.View
        ...
        style={[
          ...
          styles.heroShape,
          {
            ...
            borderLeftColor: colors.primaryDark ?? colors.primary,
          },
        ]}
      >
        <ThemedText
          type="title"
          style={[styles.title, { color: colors.primaryDark ?? colors.text }]}
        >
          {t("providers.title")}
        </ThemedText>
      ...
      {providers.map((p, i) => (
        <Animated.View
          key={p.id}
          entering={FadeInDown.duration(400).delay(80 + i * 40)}
        >
          <ProviderCard
            provider={p}
            onPress={() => {
...
const styles = StyleSheet.create({
  ...
  heroShape: { borderLeftWidth: 4, borderLeftColor: "transparent" as string },
